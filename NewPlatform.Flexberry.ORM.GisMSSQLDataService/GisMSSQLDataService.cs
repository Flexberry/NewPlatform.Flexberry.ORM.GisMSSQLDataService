namespace NewPlatform.Flexberry.ORM
{
    ...
    public class GisMSSQLDataService : MSSQLDataService
    {
        ...
        /// <param name="businessServerProvider">The provider for ICSSoft.STORMNET.Business.BusinessServer creation.</param>
            ...
            if (!string.IsNullOrEmpty(sqlFunction))
            {
                ...
                for (int i = 0; i < sqlParameters.Length; i++)
                {
                    sqlParameters[i] = value.Parameters[i] is VariableDef vd
                        ? PutIdentifierIntoBrackets(vd.StringedView)
                        : convertValue(value.Parameters[i]);
                }
